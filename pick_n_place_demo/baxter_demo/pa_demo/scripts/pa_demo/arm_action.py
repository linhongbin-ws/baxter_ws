...
# Uses pykdl to compute the joing angles as part of a dictionary with joint angle names as key
...
#-------------------------------------------------
# computerApproachPose
# Two methods:
# 1. get_cartesian_offset_pose: given a target position, and one offset, it returns, the offset position. Used to have a guarded approach to a pick/place task.
# 2. get_approach_joints_2: calls get_cartesian_offset_pose twice with different offsets, to get 2 points previous to mainpulation.
class computerApproachPose(object):
    def __init__(self,limb):
        self._limb=limb

        # get_approach_joints_2
        # This method requires two hardcoded offsets to prepare a grasp.
        # offset2 is some distance z above the target_pose, usually 1cm.
        # offset1 is the same but usually 5cm.

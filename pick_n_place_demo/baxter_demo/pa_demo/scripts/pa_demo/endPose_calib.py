#!/usr/bin/env python
#------------------------------------------------------------------------------------------------------------------------------------------------
# This program works tightly with pa_localization as of June 2016.
# It assumes a set of reference positions connected to the origin of the visual system in pa_localization.
# After that it will move to 5 different locations 2.5cm appart in x,y. But the arm will move from top to bottom visiting two points each time. 
# First point is 5cm above, 2nd point is 1cm above.
#------------------------------------------------------------------------------------------------------------------------------------------------
import sys
import argparse
import rospy
...
def main():
    ...
    for i in range(0,5):
        for j in range(0,5):
            ...
            # Move 5cm above the target pose
            arm.move_to_joint_positions(approach_J_1)
            rospy.sleep(2.0)
            
            # Move 1cm above the target pose
